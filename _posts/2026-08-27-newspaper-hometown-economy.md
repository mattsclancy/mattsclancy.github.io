...
where `α_i` is a newspaper fixed effect, `γ_t` a year fixed effect, `LocalEconGrowth_it` is the newspaper's state's total private employment growth relative to the national rate that year, `JournalismGrowth_it` is that state's newspaper-publishing employment growth relative to its own total private employment growth, and `ε_it` is an error term clustered by newspaper. The year fixed effects absorb anything common to every newspaper in a given year — national GDP, national unemployment, national political events, any shared shift in how "concerned" economic sentiment sounds. What's left is comparisons like: in a given year, was this newspaper more negative than others, when its own state's economy or its own state's newspaper industry was unusually weak that year?

| Regressor | Coefficient | Clustered SE | p |
|---|---|---|---|
| `LocalEconGrowth` (β1) | -8.37 | 2.84 | 0.003 |
...
<a href="/assets/images/hometown_analysis2_binscatter.png"><img src="/assets/images/hometown_analysis2_binscatter.png" alt="Binned scatter of economic concern (residualized on newspaper and year fixed effects and local employment growth) against relative journalism employment growth, showing a modest negative slope with substantial scatter"></a>

The underlying scatter is noisy — appropriately so, given the p-value — but the binned means trend downward as relative journalism growth improves, most visibly through the middle of the distribution where most of the data sits.

**How much does this matter in practice?** Take 1994, a year when Washington Post's newsroom shrank 8.2% relative to DC's overall economy while the New York Times's held almost flat, down just 0.7% relative to New York's economy — a 7.5-point gap, about the size of the 10th-to-90th-percentile spread of this variable across the whole panel. Applying the coefficient above to that gap alone predicts the Times's sentiment should read about 0.067 points less negative than the Post's that year — roughly **a third of a standard deviation** of the within-panel variation in economic concern. The actual gap that year: the Times came in 0.065 points less negative than the Post — almost exactly what the model implied, and in this particular year local economic conditions in the two markets were nearly identical, so journalism conditions are doing most of the work.

<a href="/assets/images/hometown_nyt_wapo_divergence.png"><img src="/assets/images/hometown_nyt_wapo_divergence.png" alt="NYT vs Washington Post journalism-growth divergence and economic concern in 1982 and 1994, showing predicted and actual concern gaps"></a>
...
Contemporaneous journalism growth: -0.89 (p=0.052). One year lagged: -0.54 (p=0.16). One year **led** — next year's journalism conditions "predicting" this year's sentiment, the placebo test — comes in at -0.12 and is nowhere near significant (p=0.67). Unlike the national analysis, the future doesn't predict the present nearly as well as the present does.
